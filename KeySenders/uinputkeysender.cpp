...
UinputKeySender::UinputKeySender()
{
    ...
    if (_device < 0) {
        std::runtime_error("Failed to open uinput device, check permissions of /dev/uinput.");
    }
    memset(&mUsetup, 0, sizeof(mUsetup));
    mUsetup.id.bustype = BUS_USB;
    mUsetup.id.vendor = 0x1234;
    mUsetup.id.product = 0x5678;
...
void UinputKeySender::emitEvent(const int &iDevice, const int &iType, const int &iCode, const int &iVal)
{
    ...
    mInputEvent.type = static_cast<__u16>(iType);
    mInputEvent.code = static_cast<__u16>(iCode);
    mInputEvent.value = iVal;
    // timestamp values below are ignored.
    mInputEvent.time.tv_sec = 0;
    mInputEvent.time.tv_usec = 0;

    write(iDevice, &mInputEvent, sizeof(mInputEvent));
}

void UinputKeySender::enableKeyEvents()
{
    const std::set<int> mDefaultKeys = {KEY_ESC,
                                        KEY_1,
                                        KEY_2,
                                        KEY_3,
                                        KEY_4,
    ...
    for (auto it : mDefaultKeys) {
        if (ioctl(_device, UI_SET_KEYBIT, it)) throw std::runtime_error("UI_SET_EVBIT ioctl failed");
    }
}
